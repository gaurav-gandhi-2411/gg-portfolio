...
import { CLUSTER_OPACITY, type RetrievalPoint } from "@/lib/triageiq-retrieval-view";

interface RetrievalSpaceGLProps {
...
/** How long the highlight takes to arrive when a different query is picked. */
const REVEAL_MS = 520;
...
/**
 * The WebGL layer of the TriageIQ retrieval space.
 *
 * Same performance contract as the Warmer viewer, and for the same reason:
 * there is no idle render loop. Frames are drawn only while a highlight is
 * arriving (about half a second after a query is picked) or on a resize.
 * A visitor who picks nothing costs zero rAF callbacks and zero GPU work,
 * which is what keeps a case-study route's Total Blocking Time out of this.
 *
 ...
 */
export default function RetrievalSpaceGL({
  points,
  emphasis,
  onUnsupported,
}: RetrievalSpaceGLProps) {
  ...
  // Animate to the new highlight whenever the selected query changes.
  useEffect(() => {
  ...
  return (
    <canvas
      ...
      data-testid="triageiq-retrieval-gl"
