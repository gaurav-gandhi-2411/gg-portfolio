...
export function CaseStudyPage({
  ...
  return (
    <main
      ...
      className="case-study-modular mx-auto w-full max-w-3xl flex-1 px-[var(--space-6)] pt-[var(--space-12)] pb-[var(--space-20)] md:pt-[var(--space-16)] lg:max-w-5xl"
    >
              ...
              {study.results?.map((result) => {
                ...
                return (
                  <div key={result.sourceRef + result.label} className="case-chip relative">
                    <dd className="case-chip-value" data-format={result.format ?? "stat"}>
              ...
              {related.map(({ product: relatedProduct }) => (
                <li key={relatedProduct.slug} className="case-block">
                  <Link
                    ...
                    className="focus-visible:outline-ring text-foreground block font-medium transition-colors duration-[var(--dur-base)] ease-[var(--ease-out-soft)] hover:text-accent focus-visible:outline-2 focus-visible:outline-offset-2 motion-reduce:transition-none"
                  >
                    {relatedProduct.name}
                  </Link>
