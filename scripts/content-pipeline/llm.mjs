...
// non-zero on any API error." This is the fix/content-pipeline-llm-fail-loud PR — Groq's eventual
// retirement of the verifier's pinned model (tracked separately in
// fix/content-pipeline-model-discovery) exposed that callLlm() never recorded ANY failure: the old
// fail-soft design swallowed every error (missing key, any non-2xx, network failure) into a bare
// `null`, which run.mjs's callers treat as "this one candidate didn't pass" — a whole model
// retirement looked identical to "the curator rejected this line." That's the same shape as
// extractor.mjs's pre-R3 commit-SHA lookup: fail-soft per-candidate is correct (one bad draft must
// never kill the run), but a systemic API failure must still surface. Fixed the same way
// extractor.mjs/github-api.mjs already do it: real errors (after retry, where retry applies) are
// recorded into the exported `apiErrors` array; run.mjs checks it after every repo is processed
// and sets a non-zero exit code — the per-candidate null-return behavior callers rely on is
// unchanged.
//
...
// R1 live-proof knob only (fix/content-pipeline-llm-fail-loud): metrics-refresh.yml's
// verifier_model_override input, when set, swaps the verifier stage's model id at call time —
// used to force a real 404 (RED) or point at a known-good model (GREEN) without hand-editing
// MODELS. Never set on the scheduled run.
