...
| `resume:indium-senior-vit` | "~70%" | Resume PDF p.1, Indium Software → Senior Data Scientist (Uber AI via Indium), ViT dual-threshold verification bullet: "Automated roughly 70% of earner document verification". Also listed, without a Source cell, in this file's "Experience bullets" 2-column table below — that table's rows all share one stated source (Resume PDF p.1) and its 2-column shape does not match `lib/provenance-parse.ts`'s 3-column row regex, so a sourceRef only listed there resolves to no provenance for `getProvenance()` callers. `HeadlineStats` (components/headline-stats.tsx) is the first caller to hit this gap directly — found via a real "no usable content/provenance.md row" build warning, not by inspection. This row is the fix for that one ref; the 2-column table's own coverage gap is unfixed and out of scope here. |
...
| Claim | Source |
|---|---|
| PyPI package `tracegauge`, `pip install tracegauge` | `token-efficiency-scorer/pyproject.toml` (`name = "tracegauge"`), badges in `README.md:6-7`. **The version was removed from this row on 2026-08-18**: it read 0.10.0, confirmed via `pip index versions tracegauge` on 2026-07-12, and the registry had since moved to 0.12.0. Version and release count are now fetched live per package by `lib/live-data.ts`'s `getPypiStats` rather than written down anywhere |
...
| `style-maitri:router-decision` | Router experiment: LLM 100% pass/~2100ms/~$0.10 per 1k vs DistilBERT 75%/31ms/~$0.05 per 1k (the reranker downstream still makes one LLM call, so it's not actually free) vs cascade 94%; kept LLM router + deterministic code guard | `reports/router_comparison.md:6-20`, `src/agents/graph.py:3345` |
...
| `tracegauge:pypi` | Installable from PyPI; the displayed version and release count are fetched live, not asserted here | Same live mechanism as `adk-tracegauge:pypi` above. **Corrected 2026-08-18:** this row read "Live on PyPI v0.10.0", cited to `pyproject.toml` and a `pip index versions` run on 2026-07-12. The registry now serves **0.12.0, 14 releases** (`pypi.org/pypi/tracegauge/json`, HTTP 200, fetched 2026-08-18), so the page had been two minor versions behind for over a month with nothing comparing the two. Found by putting a live version on the card and noticing it disagreed with the case-study body |
...
### adk-tracegauge (added 2026-08-18)

Source repo `gaurav-gandhi-2411/adk-tracegauge` at commit
`4c62769` (2026-08-18). The published surface is cited against the live PyPI JSON API rather
than the repo, deliberately: the working tree is at `0.5.0` and the registry's latest is
`0.4.1`, so citing `pyproject.toml` would have put a version on the site that nobody can
install. Every version and count on the case study comes from what the registry actually
serves.
...
| `adk-tracegauge:real-cv` | Real within-case CV 0.1566 measured by running the same 36-case eval set twice against `ollama_chat/qwen2.5:7b`; power to detect a true 10% cost regression is 28.45% [26.52%, 30.47%] at n=30 and 32.25% [30.24%, 34.33%] at n=36, and 92.25%/96.35% for a 25% effect | `README.md:131`; `scripts/measure_within_case_cv_ollama.py`, `scripts/measure_q1_within_case_power.py`; `docs/audit/Q1_WITHIN_CASE_CV.md`. Domain of validity stated at source: one eval set, one local model, two repeats per case |
| `adk-tracegauge:not-evaluated` | An invocation whose cost could not be established reports `NOT_EVALUATED` rather than a fabricated pass or a silent $0.00; local-model prefixes are fail-closed behind an explicit opt-in because Ollama Cloud shares the prefix | `README.md:305,330,364,373` |
| `adk-tracegauge:dependency-drop` | The `tracegauge` PyPI dependency was removed at Phase 4 R5 after an audit found ~55 lines of arithmetic plus two internal-only dataclasses were the only things ever imported from it | `README.md:377` |
| `adk-tracegauge:python-matrix` | CI runs the suite on Python 3.10, 3.11, 3.12, 3.13 and 3.14, against both `google-adk` releases the version range admits | `.github/workflows/ci.yml:30` and the matrix comment at `:28-29` |
| `adk-tracegauge:exit-codes` | `check` exits 0 pass, 1 regression, 3 insufficient data; `adk eval`'s own process exit code does not reflect PASSED/FAILED, verified live at source as 0 in both a passing and a failing run | `README.md:36,56,190,363` |
...
| `search:eval-limitations` | n=28 queries over the 13 candidate projects the catalog held at the time; random 3-guess baseline Recall@3 ≈ 23.08%, every tier clears it by 60+ points (real signal) but near-100% scores don't by themselves prove sophisticated retrieval on a catalog this distinct; the 28 queries are LLM-generated, not sourced from real recruiters/search logs/usage; and the catalog has since grown without the comparison being re-run | `evals/project-search/README.md` (both limitations stated in full, with the exact random-baseline arithmetic). **Third limitation added 2026-08-18**, when adk-tracegauge took the catalog to 14: the surface copy read "runs on 28 queries against 13 candidate projects" in the present tense, which would have quietly become a wrong statement about the current catalog rather than a right one about a past run. Re-running the comparison is not cheap (two of its four tiers need multi-megabyte models), so the honest move was to date the claim, not restate it over a denominator it never measured |
